...
func NewInvitationHandler(service InvitationService, orgRepo OrganizationRepository) *InvitationHandler {
...
// @Security    BearerAuth
// @Success     200 {object} utils.APIResponse{data=Organization} "Successfully joined the organization"
...
func (h *InvitationHandler) AcceptInvitation(c *gin.Context) {
	...
	org, err := h.service.AcceptInvitation(req.Token)
	if err != nil {
		switch err.Error() {
	...
	utils.SendSuccess(c, "Successfully joined the organization", org)
}
...
// @Security    BearerAuth
// @Success     200 {object} utils.APIResponse{data=[]OrganizationInvitation} "Success"
...
// @Router      /organizations/invitations [get]
func (h *InvitationHandler) GetInvitations(c *gin.Context) {
	...
	if err != nil {
		utils.SendError(c, http.StatusBadRequest, "Invalid organization ID")
		return
	}
	...
	requesterRole, err := h.getRequesterRole(requesterID, uint(orgID))
	if err != nil || !requesterRole.CanInvite() {
		utils.SendError(c, http.StatusForbidden, "Insufficient permission")
		return
	}

	invitations, err := h.service.GetPendingInvitations(uint(orgID))
	if err != nil {
...
func (h *InvitationHandler) RevokeInvitation(c *gin.Context) {
	...
	err = h.service.RevokeInvitation(token, requesterID, uint(orgID))
	if err != nil {
...
func (h *InvitationHandler) getRequesterRole(userID, orgID uint) (models.Role, error) {
	return h.orgRepo.GetMemberRole(userID, orgID)
}
